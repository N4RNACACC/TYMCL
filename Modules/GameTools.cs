...
namespace TYMCL.Modules
{
    internal class GameTools
    {
        ...
        {
            Logger.Log.Info("正在启动游戏...");

            var gameCore = "1.12.2";// 游戏核心
            Logger.Log.Info("游戏核心: " + gameCore);

            var account = new OfflineAuthenticator().Authenticate("PLAYER_NAME"); // 账户验证器
            Logger.Log.Info("账户信息: " + account.Name + " " + account.Uuid);

            Logger.Log.Info("构建启动配置...");
            ...
            Logger.Log.Info("最大内存分配: " + launchConfig.MaxMemorySize + "MB");
            Logger.Log.Info("最小内存分配: " + launchConfig.MinMemorySize + "MB");
            Logger.Log.Info("Java信息: " + launchConfig.JavaPath);

            Logger.Log.Info("启动游戏...");
            ...
            {
                Logger.Log.Info("游戏进程已退出");
            }
            else
            {
                Logger.Log.Info($"游戏进程正在运行 (PID: {process.Process.Id})");
            }
            void OnExited(object s, EventArgs e)
            {
                int exitCode = process.Process.ExitCode;
                Logger.Log.Info("游戏进程已退出-返回值: "+ exitCode );
