...
public partial class StartupWindow : Window
{
    ...
    {
        Logger.Log.Info("启动器-初始化","开始初始化流程");
        ...
        {
            Logger.Log.Info("启动器-初始化","环境检查不通过");
            // 显示错误界面并等待用户选择
        ...
        {
            CheckEnvIcon.Text = "\uf00c;";
            Logger.Log.Info("启动器-初始化", "环境检查通过");
            // 更新检查
            await Update_Check();
    ...
    private async Task ShowErrorAndWaitForUserDecision() // 显示错误界面并等待用户决定
    {
        ...
        errorControl.ContinueRequested += () => {
            ...
            Logger.Log.Info("启动器-初始化", "继续初始化流程");
            tcs.SetResult(true);
        };

        errorControl.ExitRequested += () => {
            Logger.Log.Info("启动器-初始化", "退出App");
            tcs.SetResult(false);
        };
    ...
    private void ThemeImage_Set() // 设置启动主题图
    {
        ...
        {
            Logger.Log.Info("启动器-初始化", "未设置自定义启动主题图-使用默认主题图");
        }
    }

    private async Task Update_Check() // 更新检查
    {
        Logger.Log.Info("启动器-初始化", "检查可用更新");
        await Task.Delay(5000); //
        Logger.Log.Info("启动器-初始化", "更新检查-完成");
        CheckUpdateIcon.Text = "\uf00c";
    }

    private async Task Environment_Check() // 环境检查
    {
        Logger.Log.Info("启动器-初始化", "开始环境检查");
        // APP目录
        var appDir = Path.GetFullPath(AppContext.BaseDirectory);
        Logger.Log.Info("启动器-初始化", "APP目录:" + appDir);
        //系统检查
        var osVersion = Environment.OSVersion.VersionString;
        Logger.Log.Info("启动器-初始化","OS:" + osVersion);

        // 发布类型检查
        Logger.Log.Info("启动器-初始化", "----------发布类型检查--------");
#if DEBUG
        Logger.Log.Info("启动器-初始化","App发布类型:Debug");
        Logger.Log.Info("启动器-初始化","App处于开发环境");
#else
        Logger.Log.Info("启动器-初始化", "App发布类型:Release");
#endif
        Logger.Log.Info("启动器-初始化", "----------------------------");

        Logger.Log.Info("启动器-初始化", "----------Java检查----------");
        // Java检查
        await GameTools.JavaWatch();
        Logger.Log.Info("启动器-初始化", "----------------------------");

        Logger.Log.Info("启动器-初始化", "---------App目录检查---------");
        // 临时目录索引
        var tempPaths = new[]
        {
            Path.GetTempPath(),
        ...
        foreach (var dir in appDirs)
        {
            ...
            {
                Logger.Log.Warn("启动器-初始化", $"目录不存在: {dir}");
                try
                {
                    Directory.CreateDirectory(dir);
                    Logger.Log.Info("启动器-初始化", $"已创建目录: {dir}");
                }
                catch (Exception ex)
                {
                    var message = $"无法创建必要目录: {dir}\n错误: {ex.Message}";
                    Logger.Log.Error("初始化", message);
                    _environmentIsSafe = false;
                    CheckEnvIcon.Text = "!";
                    return;
                }
            ...
            {
                Logger.Log.Info("启动器-初始化",$"目录已验证: {dir}");
            }
        }
        Logger.Log.Info("启动器-初始化","----------------------------");
    }
